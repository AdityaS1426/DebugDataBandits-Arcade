// Grabs values from the submitted form in the URL.
const words = new URLSearchParams(window.location.search);

// Cleans up & capitalizes the names of the animals.
...
<p>The <span class="word" title="id: animal-1">${firstAnimal}</span> was very <span class="word" title="id: adj-1">${adj1}</span> with the idea of running a race with the <span class="word" title="id: animal-2">${secondAnimal}</span>, but for the fun of the thing he agreed. So the <span class="word" title="id: animal-3">${thirdAnimal}</span>, who had consented to act as judge, marked the distance yelled, "<span class="word" title="id: quote">${quote}</span>".</p>
